...
public:
  ...
  size_t Get(const TAirport& airport) const {
    return airlines.at(static_cast<uint32_t>(airport));
  }

  // добавить данный элемент
  void Insert(const TAirport& airport){
    ++airlines[static_cast<uint32_t>(airport)];
  }

  // удалить одно вхождение данного элемента
  void EraseOne(const TAirport& airport){
    if(airlines[static_cast<uint32_t>(airport)]){
      --airlines[static_cast<uint32_t>(airport)];
    }
  }
  ...
  void EraseAll(const TAirport& airport){
    airlines[static_cast<uint32_t>(airport)] = 0;
  }
  ...
  Items GetItems() const {
    ...
    for(int i = 0; i < SIZE; ++i){
      result[i] = {static_cast<TAirport>(i), airlines[i]};
    }
    return result;
  }

private:
  array<int, SIZE> airlines;
};
